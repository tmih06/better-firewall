...
and `protection-benchmarks` CI artifacts, run
`python3 scripts/charts/charts.py <downloaded-artifact-dir>` — a directory
containing `summary.json` and `protection-benchmarks.txt` — and commit the
updated SVGs. `make charts` regenerates them from the committed snapshot.
